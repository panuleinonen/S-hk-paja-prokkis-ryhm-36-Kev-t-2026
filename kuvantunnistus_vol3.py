...
GPIO.setmode(GPIO.BCM) #GPIO.BCM = universal, GPIO.BOARD = Board specific. As to why BCM works instead of Board, I don't know.


#Sets pins using a list rather than a unclean block of GPIO.setup(pin, GPIO.I/O) lines.
for pin in (DIR_PIN_X, STEP_PIN_X, DIR_PIN_Y, STEP_PIN_Y, HALF_STEP):
    GPIO.setup(pin, GPIO.OUT)

GPIO.output(HALF_STEP, GPIO.HIGH)

#A4988Nema(dir_pin, step_pin, (MS1, MS2, MS3), motor_type) motor_type : "DRV8825"
...
# Variables that can be accessed by each thread
error_lock = threading.Lock()
latest_error = {"x": 0, "y": 0}  # updated by inference thread
